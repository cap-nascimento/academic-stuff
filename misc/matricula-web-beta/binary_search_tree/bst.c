...
void bst_access_modify_subject(bst_t* t, bst_node* root, void* data_sub, void* data_stu){
	if(t->comparator(data_sub, root->data) < 0){
		bst_access_modify_subject(t, root->left, data_sub, data_stu);
	}else if(t->comparator(data_sub, root->data) > 0){
		bst_access_modify_subject(t, root->right, data_sub, data_stu);
	}else{
		register_student(root->data, data_stu);
	}
}

void bst_access_extract_subject(bst_t* t, bst_node* root, void* data_sub, dlist_t* l){
	if(t->comparator(data_sub, root->data) < 0){
		bst_access_extract_subject(t, root->left, data_sub, l);
	}else if(t->comparator(data_sub, root->data) > 0){
		bst_access_extract_subject(t, root->right, data_sub, l);
	}else{
		extract_student(root->data, l);
	}
}

void* bst_access(bst_t* t, bst_node* root, void* data){
	if(t->comparator(data, root->data) < 0){
		return bst_access(t, root->left, data);
	}else if(t->comparator(data, root->data) > 0){
		return bst_access(t, root->right, data);
	}
	return root->data;
}
...
void traverse_in_order_student(bst_node* root){
	if(root != NULL){
		traverse_in_order_student(root->left);
		process_student(root->data);
		traverse_in_order_student(root->right);
	}
}

void traverse_in_order_subject(bst_node* root){
	if(root != NULL){
		traverse_in_order_subject(root->left);
		process_subject(root->data);
		traverse_in_order_subject(root->right);
	}
}
